...
void App::input() {
	...
	one_line.frequency = getRightNumber("Введите частоту (1-30): ");
	one_line.speed = getRightNumber("Введите скорость (1-30): ");
	one_line.lenght = getRightNumber("Введите длину (1-30): ");
	one_line.epilepsy_flag = getRightText("Эпилепсия (yes/no): ");
...
void App::spawnLines() {
	...
	if (getTimeSec() > time_now_sec)
	{
		...
		for (int i = 0; i < one_line.frequency; i++)
			creation_lines.push_back(time_now_ms + rand() % 1000); //линии которые нужно создать
	}
}
...
void App::createLines() {
	...
	for (int i = 0; i < creation_lines.size(); i++) {
		if (getTimeMS() >= creation_lines[i] && existing_lines < one_line.frequency && free_places.size() != 0)
		{
			...
			one_line.draw_time = getTimeMS() + one_line.interval; //время в которое надо наривсовать символ
